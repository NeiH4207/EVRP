...
void GreedySearch::greedy_search(Solution &sol){
    cout << "Running greedy search\n";
    sol.show();
    cout << "Clustering\n";
    clustering(sol);
    sol.show();
    cout << "Balancing capacity\n";
    balancing_capacity(sol);
    sol.show();
    cout << "Local search\n";
    local_search(sol);
    sol.show();
    cout << "Inserting charging stations\n";
    ...
    for(int i = length; i - 2 > 0; i--){
        ...
        for(int j = 0, to; j < sz; j++){
            ...
            if (direction == "backward") {
                station = nearest_station_back(from, to, energy);
            } else {
...
void GreedySearch::insert_charging_stations(Solution &sol){
    ...
    int full_length = 0;
    sol.full_path[full_length++] = 0;
    
    cout << "Number of tours: " << sol.get_num_of_tours() << "\n";
